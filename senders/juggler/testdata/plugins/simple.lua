function run()
    local result = {}
    for _, v in pairs(payload) do for _, level in pairs {"CRIT", "WARN", "INFO", "OK"} do
        check = conditions[level]
        if check then for _, case in pairs(check) do
            if v.Tags.aggregate and case:match("${%s-"..v.Tags.aggregate.."%s-}") then
                print("Check matched", case)
            end
        end end
    end end
    return result
